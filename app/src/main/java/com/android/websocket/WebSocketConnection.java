...
public class WebSocketConnection implements WebSocket {
    ...
    public boolean prevConnected() {
        return mPrevConnected;
    }
    ...
    public void connect(String wsUri, String[] wsSubprotocols,
            ...
            List<BasicNameValuePair> headers) throws WebSocketException {
    ...
    protected boolean scheduleReconnect() {
        /**
         * Reconnect only if: - connection active (connected but not disconnected) - has previous
         * success connections - reconnect interval is set
         */
    ...
    protected void createHandler() {
        ...
        mMasterHandler = new Handler(Looper.getMainLooper()) {
            ...
            public void handleMessage(Message msg) {
                ...
                if (msg.obj instanceof WebSocketMessage.TextMessage) {//收到文本消息
                ...
                } else if (msg.obj instanceof WebSocketMessage.RawTextMessage) {//收到原始消息
                ...
                } else if (msg.obj instanceof WebSocketMessage.BinaryMessage) {//收到二进制消息
                ...
                } else if (msg.obj instanceof WebSocketMessage.Ping) {//收到心跳消息
                ...
                } else if (msg.obj instanceof WebSocketMessage.Pong) {//收到心跳消息
                ...
                } else if (msg.obj instanceof WebSocketMessage.Close) {//收到关闭消息

                    WebSocketMessage.Close close = (WebSocketMessage.Close) msg.obj;

                    if (DEBUG)
                        Log.d(TAG, "WebSockets Close received (" + close.mCode + " - "
                                + close.mReason + ")");
                ...
                } else if (msg.obj instanceof WebSocketMessage.ServerHandshake) {//收到服务端的握手消息
                ...
                } else if (msg.obj instanceof WebSocketMessage.ConnectionLost) {//收到连接丢失消息
                    ...
                    failConnection(WebSocketConnectionHandler.CLOSE_SERVER_ERROR, "Server error "
                            + error.mStatusCode + " (" + error.mStatusMessage + ")");
