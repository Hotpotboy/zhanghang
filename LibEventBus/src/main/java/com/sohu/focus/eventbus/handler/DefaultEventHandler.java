...
public class DefaultEventHandler implements EventHandler {
    ...
    public void handleEvent(Subscription subscription, Object event) {
        try {
            // 执行
            subscription.targetMethod.setAccessible(true);
            boolean reuslt = (boolean) subscription.targetMethod.invoke(subscription.subscriber.get(), event);
            subscription.isGoOn = reuslt;
